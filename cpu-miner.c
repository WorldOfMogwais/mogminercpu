...
Options:\n\
  -a, --algo=ALGO       specify the algorithm to use\n\
                          mogwai       modified NeoScrypt  *default\n\
...
static bool submit_upstream_work(CURL *curl, struct work *work)
{
    ...
    if (have_stratum) {
        ...
        } else if (opt_algo == ALGO_NEOSCRYPT || opt_algo == ALGO_MOGWAI) {
            data_size = 80;
        }
...
static void stratum_gen_work(struct stratum_ctx *sctx, struct work *work)
{
    ...
    if (jsonrpc_2) {
        ...
        if (opt_algo == ALGO_NEOSCRYPT || opt_algo == ALGO_MOGWAI) {
            /* reversed endian */
            for (i = 0; i <= 18; i++)
